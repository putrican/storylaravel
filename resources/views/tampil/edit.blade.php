    ...
    <div class="container mt-3">
        <div class="row">
            <div class="col-sm-8 col-md-6">
                ...
                <hr>
                <form action="{{ url('/admin/'.$mahasiswa->id) }}" method="POST">
                    @method('PATCH')
                    @csrf
                    <div class="form-group">
                        <label for="nik">Nik</label>
                        <input type="text" name="nik" id="nik" class="form-control @error('nik') is-invalid @enderror" value="{{ old('nik') ?? $mahasiswa->nik }}">
                        @error('nik')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('nomor_telepon') is-invalid @enderror" id="nomor_telepon" name="nomor_telepon" value="{{ old('nomor_telepon') ?? $mahasiswa->telepon->nomor_telepon }}">   
                        @error('nomor_telepon')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="bagian_id" name="bagian_id">
                            @foreach ($bagian as $bagian)
                                <option value="{{ $bagian->id }}" {{ old('bagian_id') ??$mahasiswa->bagian_id == $bagian->id ? 'selected' : '' }}>
                                    {{ $bagian->nama_bagian }}
                                </option>
                            @endforeach                       
                        </select>   
...
@endsection
 @section('js')
     <script>
        $(".edit-hobi").select2().val({!! json_encode($mahasiswa->hobi()->allRelatedIds()) !!}).trigger('change');
     </script>
 @endsection
